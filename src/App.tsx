  ...
  useEffect(() => {
    ...
    const evaluateOfflineStatus = () => {
      ...
      Object.entries(devicePingsRef.current).forEach(([asset_id, last_ping]) => {
        const pingDate = new Date(last_ping as string);
    ...
    const assetsTableRows = assets.map(a => `
      <tr class="border-b border-slate-100 dark:border-slate-800 hover:bg-slate-50 dark:hover:bg-slate-900/50 transition-colors">
        ...
        <td class="px-4 py-3 text-xs">
          <span class="inline-flex items-center gap-1 px-2 py-0.5 rounded-full font-bold text-xs ${a.status === 'Em Uso' ? 'bg-emerald-50 text-emerald-700 dark:bg-emerald-900/20 dark:text-emerald-400' :
    ...
    const statusChartRowsHTML = statusList.map(s => `
      <div class="space-y-1">
        ...
        <div class="w-full bg-slate-100 dark:bg-slate-800 h-2.5 rounded-full overflow-hidden">
          <div class="${s.name === 'Em Uso' ? 'bg-emerald-550' :
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
          ...
          <div class="print-card bg-slate-900/50 border border-slate-800 p-6 rounded-2xl flex flex-col justify-between">
            ...
            <div class="p-3 bg-slate-950/40 border border-slate-800 rounded-xl text-xs text-slate-500 flex justify-between">
              ...
              <span class="font-bold text-emerald-400">${statusList.find(s => s.name === 'Em Uso')?.percentage || '0'
      }% ativo</span>
            </div>
          </div>
        </div>
  ...
  <div id="report-container" class="hidden flex-1 px-6 py-10 md:px-12 max-w-6xl mx-auto w-full space-y-10">
    ...
    <div class="grid grid-cols-2 md:grid-cols-4 gap-6">
      ...
      <div class="print-card bg-slate-900/50 border border-slate-850 p-5 rounded-2xl">
        ...
        <span class="text-3xl font-extrabold text-emerald-400">${statusList.find(s => s.name === 'Em Uso')?.count || 0
      }</span>
      </div>
    </div>
    ...
    return (
      <AuthScreen
        ...
        onResetComplete={() => {
          ...
          // session will persist, proceeding to main app naturally
        }}
      />
    );
  }
